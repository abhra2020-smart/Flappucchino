...
def main():
	...
	font_80 = pygame.font.Font('data/fonts/font.otf', 80)
	# get some images
	...
	while splashScreenTimer < 100:
		...
		creator = font_80.render("ABHRA2020-SMART", True, (171, 145, 123))
		text = font_small.render("POWERED BY PYGAME", True, (171, 145, 123))
		DISPLAY.blit(creator, (DISPLAY.get_width() / 2 - creator.get_width() / 2,
		                       DISPLAY.get_height() / 2 - creator.get_height() / 2 + math.sin(time.time() * 5) * 5 - 25))
		DISPLAY.blit(text, (DISPLAY.get_width() / 2 - text.get_width() / 2,
		                    (DISPLAY.get_height() / 2 - text.get_height() / 2) + (math.sin(time.time() * 5) * 5 - 25) + 75))
	...
	while titleScreen:
		...
		if (clicked and checkCollisions(mouseX, mouseY, 3, 3, DISPLAY.get_width() / 2 - retry_button.get_width() / 2, 288,
		...
		creator = font_small.render("START", True, (0, 0, 0))
		DISPLAY.blit(creator, (DISPLAY.get_width() / 2 - creator.get_width() / 2, 292))
	...
	while True:
		...
		for bean in beans:
			...
			if (checkCollisions(player.position.x, player.position.y, player.currentSprite.get_width(), player.currentSprite.get_height(),
		...
		for button in buttons:
			...
			if clicked and not dead and checkCollisions(mouseX, mouseY, 3, 3, buttonX, buttonY, button.sprite.get_width(),
				...
				if (beanCount >= button.price):
					...
					if (buttons.index(button) == 2):
						...
						for i in range(beanMultiplier):
							beans.append(Bean())
							beans[-1].position.xy = random.randrange(0,
							                                         DISPLAY.get_width() - bean.sprite.get_width()), player.position.y - DISPLAY.get_height() - random.randrange(
								0, 200)
